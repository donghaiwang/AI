%% Q学习的大脑，包括：动作的选择、学习
% QLearningTable(1, 0.1, 0.01, 0.9);
classdef QLearningTable
    properties
        actions;        % 采取的行动
        learningRate;   % 学习率
        gamma;          % 奖励的衰减因子
    ...
    methods 
        function obj = QLearningTable(actions, learningRate, rewardDecay, epsilonGreedy)
            obj.actions = actions;
        ...
        function obj = learn(obj, state, action, reward, nextStatus)
            QPredict = obj.QTable{state, action};
            if nextStatus ~= 13     % 下一个状态不是终点
                QTarget = reward + obj.gamma * max(obj.QTable{nextStatus,:});
            else
